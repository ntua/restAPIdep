...
for line in data1:
    if '"name":' in (line):
        ...
        if querykey != '':
                            if querykey in res_set:
                                keychain = querykey
                                for tempid in res_id[keychain]:
                                    if id != tempid[0] and keychain not in excluded_keys:
...
fileout = open('output_files/'+f.replace('.json','')+'_dependencies_'+filename+'_by_name.txt', 'w')
fileout.writelines('----------------------------------------------\n')
...
fileout2 = open('output_files/'+f.replace('.json', '')+'_dependencies_attributes_'+filename+'_by_name.txt', 'w')
fileout2.writelines('----------------------------------------------\n')
